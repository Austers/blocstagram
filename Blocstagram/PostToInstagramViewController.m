...
#pragma mark - UICOllectonView delegate and data source
...
[self.photoFilterOperationQueue addOperationWithBlock:^{
    CIFilter *kaleidoFilter = [CIFilter filterWithName:@"CIColorInvert"];
        
        if (kaleidoFilter) {
            [kaleidoFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            [self addCIImageToCollectionView:kaleidoFilter.outputImage withFilterTitle:NSLocalizedString(@"Invert", @"ColorInvert filter")];
        }
...
[self.photoFilterOperationQueue addOperationWithBlock:^{
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (clampFilter) {
            [clampFilter setValue:sourceCIImage forKey:kCIInputImageKey];
        
            CIImage *result = clampFilter.outputImage;
            ...
            [self addCIImageToCollectionView:result withFilterTitle:NSLocalizedString(@"Combo", @"Combination image")];
        }
